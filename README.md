...
X The `Doctor` class needs an instance method, `#patients`, that iterates over
  that doctor's `Appointment`s and collects the patient that belongs to each
  `Appointment`s.

**The `Appointment` model:**

X The `Appointment` class needs a class variable `@@all` that begins as an empty array.
X The `Appointment` class needs a class method `.all` that lists each `Appointment`
  in the class variable.
X An `Appointment` should be initialized with a date (as a string, like `"Monday, August 1st"`), a patient, and a doctor. The `Appointment` should be saved in the
  `@@all` array.

**The `Patient` model:**

X The `Patient` class needs a class variable `@@all` that begins as an empty array.
X The `Patient` class needs a class method `.all` that lists each patient in the
  class variable.
X A patient is instantiated with a name and be saved in the `@@all` array.
X The `Patient` class needs an instance method, `#new_appointment`, that takes in
...
X The `Patient` class needs an instance method, `#appointments`, that iterates
  through the `Appointment`s array and returns `Appointment`s that belong to the
  patient.
X The `Patient` class needs an instance method, `#doctors`, that iterates over
